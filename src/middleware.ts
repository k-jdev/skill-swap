...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    // An unhandled throw here 500s every request, public pages included, so
    // the failure is contained. Protected routes fail closed: if the session
    // cannot be verified, the visitor is treated as anonymous.
